...
class PolicyNetContinuous(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    dist = Normal(mu, std)
    normal_sample = dist.rsample() # rsample是重参数化采样
    log_prob = dist.log_prob(normal_sample)
    action = torch.tanh(normal_sample)
    # 计算tanh_normal分布的对数概率密度
    # 是否由必要？毕竟从normal_sample到最终的action是一一映射的,
    # 并且概率分布形状没有变化, 只是不同action值反求到normal_sample，然后再查询概率密度
...
class SACContinuous:
  ...
  def __init__(self, state_dim, hidden_dim, action_dim, action_bound,
    ...
    self.target_critic_1.load_state_dict(self.critic1.state_dict())
    self.target_critic_2.load_state_dict(self.critic2.state_dict())

    self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=actor_lr)
    self.critic_1_optimizer = torch.optim.Adam(self.critic1.parameters(), lr=critic_lr)
    self.critic_2_optimizer = torch.optim.Adam(self.critic2.parameters(), lr=critic_lr)
    ...
    self.target_entropy = target_entropy # SAC目标熵
    self.gamma = gamma
    self.tau = tau
    self.device = device
  ...
  def update(self, transition_dict):
    ...
    # 更新目标Q网络
    self.soft_update(self.critic1, self.target_critic_1)
    self.soft_update(self.critic2, self.target_critic_2)
